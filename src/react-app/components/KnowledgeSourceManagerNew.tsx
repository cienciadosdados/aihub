...
interface SourceWithProgress extends KnowledgeSource {
  progress?: {
    percentage: number;
    message: string;
    stage: string;
...
export default function KnowledgeSourceManager({ agentId, isOpen, onClose, user }: KnowledgeSourceManagerProps) {
  const [sources, setSources] = useState<SourceWithProgress[]>([]);
  ...
  const fetchSourcesWithProgress = async () => {
    try {
      // Send log to backend for centralized logging
      fetch('/api/log', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ level: 'info', message: `[FRONTEND-POLLING] Fetching progress for agent ${agentId}`, timestamp: new Date().toISOString() })
      }).catch(() => {}); // Silent fail
      const response = await fetch(`/api/agents/${agentId}/knowledge-sources/status`, {
        headers: {
          "X-User-Data": JSON.stringify(user)
        }
      });
      if (response.ok) {
        ...
        fetch('/api/log', {
          method: 'POST', 
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ level: 'info', message: `[FRONTEND-POLLING] Received ${data.sources?.length || 0} sources`, data: data, timestamp: new Date().toISOString() })
        }).catch(() => {});
        const sourcesWithProgress = data.sources.map((source: any) => {
          const progress = source.status === 'processing' ? {
            percentage: source.progress_percentage || 0,
            message: source.progress_message || 'Processing...',
          ...
          if (progress) {
            ...
            fetch('/api/log', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ level: 'info', message: `[FRONTEND-PROGRESS] Source ${source.id}: ${progress.percentage}% - ${progress.message}`, sourceId: source.id, progress: progress, timestamp: new Date().toISOString() })
            }).catch(() => {});
          }
        ...
        setSources(sourcesWithProgress);
        
        // Keep polling if any sources are pending or processing
        const hasActive = sourcesWithProgress.some((s: any) => s.status === 'processing' || s.status === 'pending');
        if (!hasActive && pollingIntervalRef.current) {
          console.log('No sources active, stopping polling');
          stopPolling();
        }
      }
    } catch (error) {
      console.error("Failed to fetch knowledge sources status:", error);
    }
  };
  ...
  const handleAddSource = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Start polling for the new source
        if (!pollingIntervalRef.current) {
          startPolling();
        }
      } else {
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {sources.map((source) => (
                ...
                >
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-2 text-sm text-white/60">
                          ...
                          {source.progress && source.progress.percentage > 0 && (
                            <span className="text-blue-400">({source.progress.percentage}%)</span>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Progress Bar for Processing Sources */}
                  {source.status === 'processing' && source.progress && (
                    <div className="mt-3 space-y-2">
                      <div className="flex justify-between text-xs">
                        <span className="text-blue-400 font-medium">{source.progress.stage || 'Processing'}</span>
                        <span className="text-blue-400">{source.progress.percentage}%</span>
                      </div>
                      ...
                      {source.progress.estimated_time_remaining_ms && (
                        <div className="text-xs text-white/50">
                          Est. {Math.ceil(source.progress.estimated_time_remaining_ms / 1000)}s remaining
                        </div>
                      )}
                    </div>
                  )}

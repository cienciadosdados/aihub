    ...
    try {
      ...
      if (response.ok) {
        ...
        const sourcesWithProgress = data.sources.map((source: any) => ({
          ...source,
          progress: source.status === 'processing' ? {
        ...
        const hasProcessing = sourcesWithProgress.some((s: any) => s.status === 'processing');
        if (!hasProcessing && pollingIntervalRef.current) {
          console.log('No sources processing, stopping polling');
          stopPolling();
        }
      }
    } catch (error) {
